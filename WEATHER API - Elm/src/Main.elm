...
import Model.WeatherData as WeatherData exposing (ApiWeatherData, HourlyDataPoint, toHourlyDataPoints, getTemperature)
import Model.WeatherItems exposing (SelectedWeatherItems, WeatherItem(..), set)
import Task
import Time
...
import View.WeatherItems exposing (..)
import View.Week
        ...
        ( newState, cmd ) =
            case ( msg, model.state ) of
                ...
                ( ChangeWeatherItemSelection item newValue, Model.HaveWeatherAndTime data ) ->
                    ( Model.HaveWeatherAndTime { data | selectedItems = (data.selectedItems |> Model.WeatherItems.set item newValue) }, Cmd.none)
...
getTemp : {time : Time.Posix, temperature : Float, precipitation : Float} -> Float
getTemp {temperature} = 
    temperature

getPrecipitation : {time : Time.Posix, temperature : Float, precipitation : Float} -> Float
...
getChartData weather =
  let
    ...
    minTemp = hourlyPoints |> List.sortBy .temperature |> List.head 
    maxTemp = hourlyPoints |> List.sortBy .temperature |> lastElem 
    hovering = weather.hovering
    shownItems = View.WeatherChart.showAllItems
  in
    View.WeatherChart.ChartData now minTemp maxTemp hourlyPoints hovering shownItems
        ...
        toDailydata dp acc =
            ...
            x::xs -> 
                let
                    ...
                    tempList = List.map (\x1 -> getTemp x1) ((\(u,v)-> v) x)
                    highTemp = List.maximum tempList
                    lowTemp = List.minimum tempList

                    precipitationList = List.map (\x1 -> getPrecipitation x1) ((\(u,v)-> v) x)
                    totalPrecipitaion = List.foldr (+) 0 precipitationList
                in
