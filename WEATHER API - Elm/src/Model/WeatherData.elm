...
toHourlyDataPoints weatherData =
 ...
 in
  List.map3 (\x -> \y -> \z -> HourlyDataPoint (Time.millisToPosix ((x+weatherData.utcOffset) *1000)) y z) hourly.times hourly.temperatures hourly.precipitation
    
getTemperature : Maybe HourlyDataPoint -> Maybe Float
getTemperature dataPoint = 
  case dataPoint of
    Just dp -> Just dp.temperature
    Nothing -> Nothing
