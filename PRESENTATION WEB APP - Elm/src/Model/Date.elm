module Model.Date exposing (Date, Month(..), compare, monthsInDate, compareMonth, full, month, monthToString, monthsBetween, monthsBetweenMonths, offsetMonths, onlyYear, view, year)
...
monthsBetween dA dB =
  ...
  in
    case (monthA, monthB) of
      (Just a, Just b) -> Just (abs((monthToInt b + yearB * 12) - (monthToInt a + yearA * 12)))
      (Nothing, Nothing) -> Just (12 * (yearB - yearA))
      (_, _) -> Nothing
...
compare (Date d1) (Date d2) =
  let
    x1 = monthsInDate (Date d1)
    x2 = monthsInDate (Date d2) 
  in
    if x1 > x2 then 
      GT
    else if x1 < x2 then
...
view (Date d) =
  ...
  in
    case m of
      Just mo -> text (monthToString mo ++ " " ++ String.fromInt y)
      Nothing -> text (String.fromInt y)
